package org.hcm.compile.lexing;
...
import org.hcm.compile.entity.Word;
import org.hcm.compile.entity.Error;
...
public class LexAnalyse {
			...
			if (!noteFlag) {
				if (isLetter(temp) || temp == '_') {// 判断是不是标志符
					...
					while ((index < length) && (!Word.isBoundarySign(str.substring(index, index + 1)))
							&& (!Word.isOperator(str.substring(index, index + 1))) && (str.charAt(index) != ' ') && (str.charAt(index) != '\t')
							&& (str.charAt(index) != '\r') && (str.charAt(index) != '\n')) {
						index++;
						// temp=str.charAt(index);
					}
					endIndex = index;
	...
	public String outputWordList() throws IOException {
		...
		pw1.println("行号\t\t\t单词\t\t\t类型");
		Word word;
		for (int i = 0; i < wordList.size(); i++) {
			word = wordList.get(i);
			pw1.println(word.line + "\t\t\t" + word.value + "\t\t\t" + word.type);
		}
		if (lexErrorFlag) {
			Error error;
			...
			pw1.println("错误编号\t\t所在行\t\t错误信息 \t\t错误单词");
			for (int i = 0; i < errorList.size(); i++) {
				error = errorList.get(i);
				pw1.println(error.id + "\t\t\t" + error.line + "\t\t\t" + error.info + "\t" + error.word.value);
			}
		} else {
			pw1.println("词法分析通过");
