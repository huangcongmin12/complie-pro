...
import org.hcm.compile.entity.AnalyseNode;
import org.hcm.compile.entity.FourElement;
...
public class Parser {
	...
	private void termOP(String term) {
		if (firstWord.type.equals(Word.INT_CONST) || firstWord.type.equals(Word.CHAR_CONST) || term.equals(firstWord.value)
	...
	private void nonTermOP(String nonTerm) {
		...
		case 'F':
			if (firstWord.type.equals(Word.IDENTIFIER)) {
				...
				// wordList.remove(0);
				// error=new
				// Error(errorCount,"不能进行算术运算的数据类型",firstWord.line,firstWord);
				// errorList.add(error);
				// graErrorFlag=true;
			}
			break;
